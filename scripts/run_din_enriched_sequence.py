...
EXPERIMENT_ID = "DS2.din_author_content_time"
FINAL_BASELINE_ID = "DL1.din_bce"
INCREMENTAL_BASELINE_ID = "DS1.din_multibehavior"
...
def train_and_evaluate(
    ...
    content_lookup = build_candidate_content_lookup(
        encoder, content_item_ids, content_vectors
    )
    model = EnrichedDIN(
    ...
    metrics.update(
        {
            ...
            "candidate_oov_content_enabled": False,
            "history_features": [
                "item",
                "category",
                "complete",
...
def main() -> None:
    ...
    metrics, training_history, groups, checkpoint = train_and_evaluate(
        selection_train,
        selection_eval,
        data["interactions"],
        data["items"],
    ...
    for baseline_id, baseline_groups in {
        ...
        comparisons[baseline_id] = {
            "recall": paired_bootstrap_difference(
                baseline_groups, groups, f"recall_at_{profile['recall_k']}", args.seed
            ),
            "ndcg": paired_bootstrap_difference(
                baseline_groups, groups, f"ndcg_at_{profile['recall_k']}", args.seed
            ),
        }
    final_comparison = comparisons[FINAL_BASELINE_ID]
    eligible = (
    ...
    write_json(output / "DS2_selection_results.json", selection_result)
    write_json(
        output / "DS2_selection_training_history.json",
    ...
    if not eligible:
        write_json(
            output / "DS2_frozen_test_results.json",
            {
                "profile": args.profile,
                "experiment_id": EXPERIMENT_ID,
                "test_rerun": False,
    ...
    write_json(
        output / "DS2_frozen_test_training_history.json",
